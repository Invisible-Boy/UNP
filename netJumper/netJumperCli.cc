...
void print(sockaddr_in &sockaddr) {
    char ip[1024];
    inet_ntop(AF_INET, (void*)&sockaddr.sin_addr.s_addr, ip, 1024);
...
int main(int argc, char **argv) {
    ...
    while(1) {
        ...
        if(FD_ISSET(sockfd, &nrset)) {
            ...
            char buff[1024];
            int n = recvfrom(sockfd, buff, 1023, 0, (sockaddr*)&recvaddr, &len);
            buff[n] = '\0';
            if(!flag && len == sizeof(srvaddr) && memcmp(&recvaddr, &srvaddr, len) == 0) {
                ...
                char hello[] = "hello Yvan";
                if(strcmp(buff, hello) == 0) continue;
                printf("Lily said: %s", buff);
            }
        }
        if(FD_ISSET(fileno(stdin), &nrset) && flag) {
            char buff[1024];
            int n = read(fileno(stdin), buff, 1023);
